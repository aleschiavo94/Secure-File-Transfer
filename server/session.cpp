...
int counter_incr_then_check(uint32_t& counter, uint32_t& counter_received){
		counter++;
		return (int) (counter == counter_received);
}
...
int receive_message(string& command, int sock, uint32_t& counter, unsigned char* key){
	...
	// message : <iv | counter | {command}_ksc | hmac[counter|{command}_ksc] >
...
/*		cout << endl << "message received: ";
	BIO_dump_fp(stdout, (char*)message, message_size);
	cout << endl;
*/

		//retrieve hmac[counter|{command}_ksc]
...
int execute_command(string command, int sock, uint32_t& counter, unsigned char* Ksc){
	...
	if (command == "list")
	{
	    ret = execute_list(sock, counter, Ksc);
	    if(ret == 0){
	...
	{ 
		ret = execute_upload(sock, counter, Ksc);
	    if(ret == 0){
	...
	{
		ret = execute_download(sock, counter, Ksc);
	    if(ret == 0){
	    	cerr<<"Error in execute_upload()"<<endl;
	    	return 0;
	    }
	...
	{
		ret = execute_quit(sock, counter, Ksc);
	    if(ret == 0){
	    	cerr<<"Error in execute_quit()"<<endl;
	    	return 0;
	    }
