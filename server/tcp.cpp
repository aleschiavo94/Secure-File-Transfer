...
int receive_TCP(int sock, unsigned char*& buff, uint16_t& buff_size, unsigned char* key, int keylen){
	...
	if(ret == 0){
		cout<<"-> Client closed the connection. Closing sock."<<endl;
		return -1;	
	}		
	if(ret == -1){
		perror(" !!! Length recv()");
...
int receive_cert_nonce(int sock, X509** client_cert, unsigned char** client_nonce){
	...
	X509* c_cert = d2i_X509(NULL, (const unsigned char**)&cert_buf, buff_size);
	if(c_cert == NULL){
		cerr<<" !!! Error: c_cert is NULL"<<endl;
		return 0;
	}

	cout<<"-> M1 : Client's certificate received!"<<endl;
	*client_cert = c_cert;
	...
	cout<<"-> M1 : Client's nonce received!"<<endl;
	*client_nonce = nonce_buf;
	return 1;
}
...
int send_cert_nonce(int sock, unsigned char* server_nonce){
	...
	X509* mycert = import_cert("server_cert.pem");	
	...
	cout<<"-> M2 : Server certificate sent!"<<endl;
	free(cert_buf);

	ret = send_TCP(sock, server_nonce, sizeof(uint32_t), NULL, 0);
	if(ret == 0){
		return 0;
	}
	cout<<"-> M2 : Server nonce sent!"<<endl;
