...
int secure_recv_msgfileinfo(int sock, unsigned char* key, int key_size,
	...
	if(counter_incr_then_check(counter_server, counter_received_ui)==0){
		cout << endl << "received: " << counter_received_ui << " actual: " << counter_server << endl;
...
int execute_upload(int sock, uint32_t& counter, unsigned char* key){
...
*/
		//cout << "-> File chunk nr[" << i << "] chunkorrectly received.\n"; 
